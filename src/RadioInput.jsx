export default function RadioInput({ id, name, handleChange, children }) {
	return (
		<div className="inline-flex items-center" key={id}>
			<label className="relative flex items-center cursor-pointer" htmlFor={id}>
				<input
					name={name}
					type="radio"
					className="peer h-4 w-4 cursor-pointer appearance-none rounded-full bg-white border border-slate-300 checked:border-slate-400 transition-all"
					id={id}
			...
			<label
				className="ml-2 text-slate-800 cursor-pointer text-sm pb-0.5"
				htmlFor={id}
			>
